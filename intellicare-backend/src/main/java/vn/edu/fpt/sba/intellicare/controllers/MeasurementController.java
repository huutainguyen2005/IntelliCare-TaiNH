...
public class MeasurementController {
    ...
    @PostMapping("/submit-kiosk")
    public ResponseEntity<String> recordKioskData(@RequestBody vn.edu.fpt.sba.intellicare.dto.request.KioskHardwareDataDTO request) {
        measurementService.recordKioskData(request.deviceId(), request.rawHex(), request.heightCm());
        return ResponseEntity.ok("Ghi nhận luồng Kiosk thành công");
    }
    ...
    // Gọi khi bệnh nhân bấm nút "TIẾN HÀNH CÂN" ở màn xác nhận (sau khi quét
    // CCCD) - chỉ TỪ LÚC NÀY session mới thực sự "mở cửa" nhận dữ liệu cân
    // từ ESP32. Trước đó (AwaitingStart), ESP32 gửi cân lên sẽ bị từ chối,
    // tránh chốt nhầm phiên nếu ai đó lỡ chạm cân lúc đang xác nhận danh tính.
    @PostMapping("/start-weighing")
    public ResponseEntity<?> startWeighing(@RequestBody Map<String, String> request) {
        try {
            String deviceId = request.get("deviceId");
            if (deviceId == null || deviceId.trim().isEmpty()) {
                return ResponseEntity.badRequest().body(Map.of("message", "Thiếu deviceId"));
            }

            MeasurementSessionResponseDTO session = measurementService.startWeighing(deviceId.trim());
    ...
    public ResponseEntity<?> startSessionFromQr(@RequestBody Map<String, String> request) {
        try {
            ...
            // KHỞI TẠO PHIÊN ĐO (Ở TRẠNG THÁI AwaitingStart - chờ bấm "Tiến hành cân")
