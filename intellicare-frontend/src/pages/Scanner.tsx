...
import { speakWeight, speakWeightAndHeight, formatWeight, unlockAudio } from "../utils/weightAudio";
        ...
        try {
          ...
          if (response.data.status === "Completed") {
            ...
            if (response.data.heightCm) {
              setHeightResult(response.data.heightCm + " cm");
              if (response.data.bmi) {
                setBmiResult(response.data.bmi.toString());
              }
              speakWeightAndHeight(response.data.weightKg, response.data.heightCm);
            } else {
      ...
      <main style={styles.stage}>
            ...
            {import.meta.env.DEV && (
              <button
                onClick={async () => {
                  ...
                  try {
                    await axiosClient.post("/api/measurements/submit-kiosk", {
                      deviceId,
                      rawHex: "",
                      heightCm: 170.5
                    });
        ...
        {status === "COMPLETED" && (
          <div style={styles.centerLayout}>
            ...
            {heightResult && (
              <div style={styles.secondaryResult}>Chiều cao: {heightResult}</div>
            )}
            {bmiResult && (
              <div style={styles.secondaryResult}>BMI: {bmiResult}</div>
            )}
