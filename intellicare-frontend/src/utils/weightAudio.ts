...
export function buildHeightAudioQueue(heightCm: number): string[] {
  const queue: string[] = [];
  
  const met = Math.floor(heightCm / 100);
  const cm = Math.floor(heightCm % 100);

  if (met > 0) {
    pushNaturalNumber(met, queue);
    queue.push("met");
  }

  if (cm > 0) {
    pushNaturalNumber(cm, queue);
    queue.push("centimet");
  }
...
export function speakWeightAndHeight(weightKg: number, heightCm: number): Promise<void> {
  const weightPart = buildWeightAudioQueue(weightKg);
  const heightPart = buildHeightAudioQueue(heightCm);
  
  // "Cân nặng hiện tại của bạn là {W} kg. Chiều cao của bạn là {H} mét {C} centimet."
  // Dùng các file tự đặt tên (ví dụ: can_nang_la, chieu_cao_la)
  const queue = ["phan_tich", ...weightPart, "chieu_cao_la", ...heightPart];
  return playGapless(queue);
}
